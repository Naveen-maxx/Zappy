...
export default function HostGame() {
  ...
  const showResults = async () => {
    ...
    const currentQ = questions[currentQuestionIndex];
    console.log('Processing results for question:', currentQ);

    // Fetch answers sorted by submission time (earliest first)
    ...
    if (answerError) console.error('Error fetching answers:', answerError);
    console.log('Fetched answers:', answers?.length);

    // Get the question start time to calculate response times
    ...
    const questionStartTime = gameStateData?.question_started_at
      ? new Date(gameStateData.question_started_at).getTime()
      : null;

    console.log('Question Start Time:', questionStartTime);

    const correctValue = currentQ.type === 'code-debug' ? currentQ.correctLine : (currentQ as any).correctIndex;

    if (answers) {
      // Fetch stats for distribution (even if real-time was missed or delayed)
      const counts: Record<string, number> = {};
      answers.forEach(a => {
        const idx = a.answer_index;
        counts[idx] = (counts[idx] || 0) + 1;
      });
      setQuestionStats({
      ...
      // Filter correct answers and assign points based on order
      const correctAnswers = answers.filter(a => a.answer_index === correctValue);
      const BASE_POINTS = 1000;
      const DECAY_RATE = 50; // Points lost per position after 1st

      console.log('Correct answers count:', correctAnswers.length);

      for (let i = 0; i < answers.length; i++) {
        const answer = answers[i];
        const isCorrect = answer.answer_index === correctValue;

        let points = 0;
        if (isCorrect) {
          // Find position among correct answers
          const correctPosition = correctAnswers.findIndex(a => a.id === answer.id);
          // First correct gets 1000, second gets 950, third 900, etc. (min 100)
          points = Math.max(100, BASE_POINTS - (correctPosition * DECAY_RATE));
        }

        console.log(`Processing answer ${answer.id} (participant ${answer.participant_id}): Correct=${isCorrect}, Points=${points}`);
        ...
        if (updateError) console.error('Error updating participant_answer:', updateError);

        // Identify who to award points to
        let targetParticipantIds = [answer.participant_id];

        // In Co-op mode, points go to the WHOLE team
        if (gameMode === 'coop') {
          ...
          if (teamMemberData?.team_id) {
            ...
            if (companions) {
              targetParticipantIds = companions.map(c => c.id);
            }
          }
        }

        // Award points to all identified participants
        for (const pId of targetParticipantIds) {
          // Get current participant data
          const { data: pData } = await supabase
            .from('room_participants')
            .select('score, current_streak, max_streak, total_answers, total_correct, avg_response_time_ms')
            .eq('id', pId)
            .single();

          if (pData) {
            const newCurrentStreak = isCorrect ? (pData.current_streak || 0) + 1 : 0;
            const newMaxStreak = Math.max(pData.max_streak || 0, newCurrentStreak);

            // Response time logic (only if this participant was the one who answered, or if we want to share it)
            let newAvgResponseTime = pData.avg_response_time_ms;
            if (pId === answer.participant_id && questionStartTime && answer.answered_at) {
              const answerTime = new Date(answer.answered_at).getTime();
              const responseTimeMs = answerTime - questionStartTime;
              const prevTotal = pData.total_answers || 0;
              const prevAvg = pData.avg_response_time_ms || 0;
              newAvgResponseTime = prevTotal > 0 && prevAvg > 0
                ? Math.round(((prevAvg * prevTotal) + responseTimeMs) / (prevTotal + 1))
                : responseTimeMs;
            }
            ...
            await supabase
              .from('room_participants')
              .update({
                score: pData.score + points,
                current_streak: newCurrentStreak,
                max_streak: newMaxStreak,
                total_answers: (pData.total_answers || 0) + 1,
                total_correct: (pData.total_correct || 0) + (isCorrect ? 1 : 0),
      ...
      // Refresh participants to get updated scores and streaks
      ...
      if (updatedParticipants) {
        ...
        // Update highest streak holder
        const maxStreak = Math.max(...updatedParticipants.map(p => p.max_streak || 0));
        const holder = updatedParticipants.find(p => p.max_streak === maxStreak && maxStreak > 0);
        setHighestStreakHolder(holder?.id || null);
      }

      // Update team scores for Team/Co-op modes
      if ((gameMode === 'team' || gameMode === 'coop') && teams.length > 0) {
        ...
        if (participantsWithTeams) {
          ...
          participantsWithTeams.forEach(p => {
            if (p.team_id) {
              teamScores.set(p.team_id, (teamScores.get(p.team_id) || 0) + p.score);
            }
          });
          ...
          // Refresh teams with members
          await fetchTeamsWithMembers();
        }
      }
    }

    // Update game state - disable chat after discussion
    await supabase
      .from('game_state')
      .update({
        phase: 'results',
